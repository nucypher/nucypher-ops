...
class BaseCloudNodeConfigurator:
    ...
    def __init__(self,  # TODO: Add type annotations
        ...
        self.emitter.echo(f'nucypher-ops running with config file: {str(self.config_path)}\n')
        ...
        defaults = {
            'envvars':
                [
                    ...
                    (NUCYPHER_ENVVAR_OPERATOR_ETH_PASSWORD, self.config['ethpassword']),
                ],
            'cliargs': [
            ]
        }
...
class DigitalOceanConfigurator(BaseCloudNodeConfigurator):
    ...
    def _configure_provider_params(self, provider_profile):
        ...
        self.region = os.environ.get('DIGITALOCEAN_REGION') or self.config.get('digital-ocean-region') or self.default_region

        self.emitter.echo(f'using DigitalOcean region: {self.region}, to change regions `export DIGITALOCEAN_REGION: https://www.digitalocean.com/docs/platform/availability-matrix/\n', color='yellow')

        self.sshkey = os.environ.get('DIGITAL_OCEAN_KEY_FINGERPRINT') or self.config.get('sshkey')
        if not self.sshkey:
            ...
            self.emitter.echo("it should look like `DIGITAL_OCEAN_KEY_FINGERPRINT=88:fb:53:51:09:aa:af:02:e2:99:95:2d:39:64:c1:64`\n", color="red")
